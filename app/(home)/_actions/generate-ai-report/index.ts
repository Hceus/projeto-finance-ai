...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  // manda as transações para o ChatGPT e pedir pra ele gerar um relatório com insights

  const content = `Gere um relatório com insight sobre as minhas finanças, com dicas e orientações de como melhorar minha vida financeira. As transações estão divididas por ponto e vírgula. A estrutura de cada uma é {DATA}-{TIPO}-{VALOR}-{CATEGORIA}. São elas: ${transactions.map((transaction) => `${transaction.updatedAt.toLocaleDateString("pt-BR")}-R$${transaction.amount}-${transaction.type}-${transaction.category}`).join(";")}`;
    ...
    messages: [
      {
        role: "system",
        content:
          "Você é um especialista em gestao e organizção de finanças pessoais. Você ajuda as pessoas a organizarem melhor as suas finanças.",
      },
      {
        role: "user",
        content,
